...
if ( len(args) <= 1 or
   (type(args[1]) is not str) or
   not (args[1].startswith("https://")) ):
   ...
   with YoutubeDL({}) as ydl:
      ...
      if os.path.isfile("./.__yyytp_cache") and len(args)>=3 and args[2]!="-no-cache":
...
AAC {format_info['AAC']}
   ...
   if format_info["Opus"]!=None:
      dl_opus_opt = {
         "format": format_info["Opus"],
         "outtmpl": f"{fulltitle}.mka"
      }
   ...
   if format_info["AV1"]!=None:
      dl_av1_opt = {
         "format": format_info["AV1"],
         "outtmpl": f"{fulltitle}_av1.mp4"
      }
...
try:
   ...
   target_audio=""
   #select video
   if dl_codec["AV1"]==True:
      target_video=fulltitle+"_av1.mp4"
   elif dl_codec["VP9"]==True:
      target_video=fulltitle+".webm"
   elif dl_codec["H264"]==True:
   ...
   #select audio
   if dl_codec["Opus"]==True:
      target_audio=fulltitle+".mka"
   elif dl_codec["AAC"]==True:
      target_audio=fulltitle+".m4a"
   else:
      print("No Audios")
      exit(1)

   cmd=["ffmpeg","-i",target_video,"-i",target_audio,"-c","copy",fulltitle+".mkv"]
   res=subprocess.run(cmd,capture_output=True,check=True,text=True,encoding="utf-8")
...
#for debug
if len(args)>=3 and ( args[2]=="-debug" or (args[2]=="-no-cache" and args[3]=="-debug") ):
   print(fulltitle)
...
if not os.path.isdir("./src"):
   print("./src is not found. so can't move files. Do you want to make src dir? \n y or n")
   make_src_dir=input()
